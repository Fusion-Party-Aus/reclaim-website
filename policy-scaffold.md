# Policy Scaffold Template

**Instructions for Contributors:**

This template helps you write policy proposals in the Fusion Reclaim style. Follow the structure below, providing evidence and costings wherever possible. Our policies follow the "They stole it, we're taking it back" framework.

---

## Policy Title

[Short, punchy title. Examples: "50,000 Homes", "10-Minute Buses", "End Wage Theft"]

**Policy Area:** [Housing / Transport / Workers' Rights / Healthcare / Education / Environment / etc.]

**Contact:** [Your name and email for questions]

**Status:** [Draft / Under Review / Approved]

---

## Executive Summary

[2-3 sentences explaining what this policy does and why it matters]

**Cost:** $[X] over [Y] years  
**Revenue Generated:** $[X] per year (if applicable)  
**People Helped:** [Number and description]

---

## THE THEFT: What They Stole From You

_This section explains the PROBLEM using "theft" framing. Make it personal and specific._

### The Stats

- **$[Amount]** - What this costs you per year
- **[Number]** - How many people affected
- **[Percentage]** - How much worse it's gotten
- **[Concrete example]** - A specific person/place harmed

### What Was Stolen

[Explain what people SHOULD have but don't. Examples:]

- Affordable housing (stolen by developers)
- Reliable transport (stolen by bad planning)
- Fair wages (stolen by wage theft)
- Access to healthcare (stolen by privatisation)

### The Evidence

[Provide specific examples and data:]

- **Example 1:** [Specific case study showing the problem]
- **Example 2:** [Another concrete example]
- **Data:** [Statistics, reports, academic studies]

**Sources:**

- [Link to source 1]
- [Link to source 2]
- [Link to source 3]

---

## THE SYSTEM: How They Rigged It

_This section explains WHY the problem exists. Follow the money. Show the corruption._

### How It Works

1. [Step 1 of how the corrupt system operates]
2. [Step 2]
3. [Step 3]
4. [Result: Regular people lose, donors profit]

### Who Benefits

- **[Group 1]:** [How they profit] - $[Amount] per year
- **[Group 2]:** [How they profit] - $[Amount] per year
- **Political Donations:** [Who donates to which parties]

### The Revolving Door

[Examples of politicians/bureaucrats who went from government to the companies that benefit]

**Example:**

> [Name] was [Government role] when [Policy] was passed. Now works for [Company that benefits]. Salary: $[Amount].

---

## OUR SOLUTION: Taking It Back

_This section explains what Fusion will DO. Be specific with timelines and targets._

### The Core Policy

[1-2 paragraphs explaining the solution in plain language]

**Target:** [What will be achieved]  
**Timeline:** [When it will happen]  
**Who It Helps:** [Specific groups that benefit]

### How It Works

**Phase 1: [Timeframe, e.g., Year 1-2]**

- [Specific action 1]
- [Specific action 2]
- [Specific action 3]
- **Result:** [What people will see]

**Phase 2: [Timeframe, e.g., Year 2-4]**

- [Specific action 1]
- [Specific action 2]
- [Specific action 3]
- **Result:** [What people will see]

**Phase 3: [Timeframe, e.g., Year 4+]**

- [Specific action 1]
- [Specific action 2]
- [Specific action 3]
- **Result:** [What people will see]

### Key Principles

- ✅ [Principle 1, e.g., "Infrastructure first, development second"]
- ✅ [Principle 2, e.g., "Evidence-based, not vibes"]
- ✅ [Principle 3, e.g., "Public good over private profit"]

---

## THE NUMBERS: What This Costs & What You Get

### Total Cost: $[X] Over [Y] Years

**Breakdown:**

- **[Item 1]:** $[Amount] - [What this pays for]
- **[Item 2]:** $[Amount] - [What this pays for]
- **[Item 3]:** $[Amount] - [What this pays for]
- **[Item 4]:** $[Amount] - [What this pays for]

**TOTAL:** $[Amount]

### Revenue Generated: $[X] Per Year

**Sources:**

- **[Source 1]:** $[Amount]/year - [Explanation]
- **[Source 2]:** $[Amount]/year - [Explanation]
- **[Source 3]:** $[Amount]/year - [Explanation]

**TOTAL REVENUE:** $[Amount]/year  
**Operating Cost:** $[Amount]/year  
**Annual Surplus:** $[Amount]/year

_[If policy generates surplus: "System pays for itself. Then makes money."]_  
_[If deficit: "Funded by [Land Value Tax / Progressive taxation / Other]"]_

### What You Get Immediately

- [Benefit 1]
- [Benefit 2]
- [Benefit 3]
- [Benefit 4]

### What You Get Medium Term (2-5 years)

- [Benefit 1]
- [Benefit 2]
- [Benefit 3]
- [Benefit 4]

### What You Get Long Term (5+ years)

- [Benefit 1]
- [Benefit 2]
- [Benefit 3]
- [Benefit 4]

---

## THE EVIDENCE: Why This Works

_Provide evidence from other jurisdictions, academic studies, and expert analysis._

### Case Study 1: [City/Country]

**What they did:** [Brief description]  
**Result:** [Specific outcomes with numbers]  
**Timeline:** [How long it took]  
**Evidence:** [Link to study/report]

**Lesson for Victoria:** [What we learn from this]

### Case Study 2: [City/Country]

**What they did:** [Brief description]  
**Result:** [Specific outcomes with numbers]  
**Timeline:** [How long it took]  
**Evidence:** [Link to study/report]

**Lesson for Victoria:** [What we learn from this]

### Case Study 3 (Failure): [City/Country]

**What they did wrong:** [Brief description]  
**Result:** [Why it failed]  
**Evidence:** [Link to study/report]

**Lesson for Victoria:** [What NOT to do]

### Academic Support

- **[Author, Year]:** [Key finding] ([Link to study])
- **[Author, Year]:** [Key finding] ([Link to study])
- **[Author, Year]:** [Key finding] ([Link to study])

### Expert Endorsements

> "[Quote from expert supporting this approach]"  
> — [Name], [Title], [Institution]

---

## FIRST 100 DAYS: What Happens Immediately

_What Fusion will do in the first 100 days in government. Be VERY specific._

- ✅ **Day 1:** [Specific action]
- ✅ **Week 1:** [Specific action]
- ✅ **Week 2:** [Specific action]
- ✅ **Week 4:** [Specific action]
- ✅ **Week 8:** [Specific action]
- ✅ **Week 12:** [Specific action]

**Visible Impact by Day 100:**

- [What people will see/feel has changed]
- [What people will see/feel has changed]
- [What people will see/feel has changed]

---

## COMPARISON: What Major Parties Offer

| Issue      | Labor/Liberal    | Greens           | Fusion             |
| ---------- | ---------------- | ---------------- | ------------------ |
| [Issue 1]  | [Their position] | [Their position] | **[Our position]** |
| [Issue 2]  | [Their position] | [Their position] | **[Our position]** |
| [Issue 3]  | [Their position] | [Their position] | **[Our position]** |
| [Issue 4]  | [Their position] | [Their position] | **[Our position]** |
| Total Cost | [Amount]         | [Amount]         | **$[Amount]**      |
| Timeline   | [When]           | [When]           | **[When]**         |

---

## FREQUENTLY ASKED QUESTIONS

### [Question 1, e.g., "Is this affordable?"]

[Answer with evidence and numbers]

### [Question 2, e.g., "Won't this [negative consequence]?"]

[Address the concern honestly, provide evidence it won't happen or explain mitigation]

### [Question 3, e.g., "Why hasn't this been done before?"]

[Explain the systemic reasons - usually political donations and vested interests]

### [Question 4, e.g., "How is this different from [existing policy]?"]

[Explain the differences clearly]

---

## IMPLEMENTATION DETAILS

### Legislative Changes Required

1. **[Act/Regulation 1]:** [What needs to change]
2. **[Act/Regulation 2]:** [What needs to change]
3. **[Act/Regulation 3]:** [What needs to change]

### Departmental Responsibility

- **Lead Department:** [Department name]
- **Supporting Departments:** [List]
- **Independent Oversight:** [Infrastructure Victoria / IBAC / Other]

### Key Stakeholders

**Supportive:**

- [Group 1]
- [Group 2]
- [Group 3]

**Opposition Expected From:**

- [Group 1] - [Why they'll oppose]
- [Group 2] - [Why they'll oppose]

**Strategy to Overcome Opposition:**
[Explain how to neutralize opposition - usually by making it politically unpopular to oppose]

---

## RISKS & MITIGATION

### Risk 1: [Description]

**Likelihood:** [High / Medium / Low]  
**Impact:** [High / Medium / Low]  
**Mitigation:** [What we'll do to prevent/minimise this]

### Risk 2: [Description]

**Likelihood:** [High / Medium / Low]  
**Impact:** [High / Medium / Low]  
**Mitigation:** [What we'll do to prevent/minimise this]

### Risk 3: [Description]

**Likelihood:** [High / Medium / Low]  
**Impact:** [High / Medium / Low]  
**Mitigation:** [What we'll do to prevent/minimise this]

---

## MESSAGING FRAMEWORK

### Elevator Pitch (30 seconds)

[2-3 sentences explaining this policy to someone on the street]

### The Theft (For doorknocking)

> "They stole [X] from you. Here's how..."

### The System (For social media)

> "[Group] donates to [Party]. [Party] passes [Policy]. [Group] profits $[Amount]. You lose."

### The Solution (For media interviews)

> "We'll [Action], funded by [Revenue Source], delivered by [Timeline]. This will [Benefit]."

### Soundbites

- "[Short punchy line 1]"
- "[Short punchy line 2]"
- "[Short punchy line 3]"

---

## SUPPORTING MATERIALS

### Infographics Needed

1. [Description of infographic 1, e.g., "How much wage theft costs per worker"]
2. [Description of infographic 2]
3. [Description of infographic 3]

### Social Media Content

**Thread 1:** [Topic]

- Tweet 1: [Content]
- Tweet 2: [Content]
- Tweet 3: [Content]
- Tweet 4: [Content]

**Thread 2:** [Topic]

- Tweet 1: [Content]
- Tweet 2: [Content]
- Tweet 3: [Content]
- Tweet 4: [Content]

### Video Scripts

**30-second ad:** [Script]

**2-minute explainer:** [Script outline]

---

## SOURCES & REFERENCES

### Government Reports

1. [Title] - [Department/Agency] - [Year] - [Link]
2. [Title] - [Department/Agency] - [Year] - [Link]

### Academic Studies

1. [Author] ([Year]). [Title]. [Journal]. [Link]
2. [Author] ([Year]). [Title]. [Journal]. [Link]

### News Articles

1. [Title] - [Publication] - [Date] - [Link]
2. [Title] - [Publication] - [Date] - [Link]

### NGO/Think Tank Reports

1. [Title] - [Organisation] - [Year] - [Link]
2. [Title] - [Organisation] - [Year] - [Link]

---

## CONTRIBUTOR NOTES

**Version History:**

- v1.0 - [Date] - [Author] - Initial draft
- v1.1 - [Date] - [Author] - [Changes]
- v2.0 - [Date] - [Author] - [Changes]

**Review Status:**

- [ ] Policy Committee Review
- [ ] Costings Verified by Economist
- [ ] Legal Review (if needed)
- [ ] Communications Team Review
- [ ] Final Approval

**Questions/Feedback:**
[Email or contact method for questions about this policy]

---

## STYLE GUIDE REMINDERS

✅ **DO:**

- Use "theft" framing to explain problems
- Provide specific numbers and timelines
- Show who profits from the current system
- Give concrete examples (suburbs, people, amounts)
- Explain HOW we'll pay for it
- Include evidence from other cities/countries
- Be punchy and direct
- Make it personal ("what this costs YOU")

❌ **DON'T:**

- Use jargon or technical language
- Make vague promises without timelines
- Ignore the costs
- Claim something is "free" when it's tax-funded
- Attack people (attack systems and corruption)
- Use academic/bureaucratic tone
- Say "we'll consider" or "we'll explore" (say what you WILL do)

---

## EXAMPLE: Good vs Bad Policy Writing

### ❌ BAD (Vague, No Numbers, No Timeline)

"We support affordable housing and will work with stakeholders to explore options for increasing supply through a range of mechanisms."

### ✅ GOOD (Specific, Costed, Timeline)

"We'll build 50,000 public homes over 4 years at a cost of $25 billion, funded by a $25 billion bond issue. This will house 125,000 people currently in housing stress and reduce rents by 15% across Melbourne."

---

**Questions?** Contact [policy@fusionparty.org.au] or open an issue on GitHub.

**Ready to submit?** Create a pull request with your completed policy using this template.
